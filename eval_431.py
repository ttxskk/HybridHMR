...
parser.add_argument('--config', default='data/models/model_config.json', help='Path to config file containing model architecture etc.')
...
def run_evaluation(model, dataset_name, dataset,
    ...
    if dataset_name == 'h36m-p1' or dataset_name == 'h36m-p2' or dataset_name == 'h36m':
        eval_pose = True
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        # img = batch['img_orig'][0].permute(1, 2, 0).cpu().numpy()
        ...
        if eval_pose:
            ...
            J_regressor_batch = J_regressor[None, :].expand(pred_vertices_graphcnn_431.shape[0], -1, -1).to(device)

            # Get 14 ground truth joints
            gt_keypoints_3d = batch['pose_3d'].cuda()
            gt_keypoints_3d = gt_keypoints_3d[:, cfg.J24_TO_J14, :-1]
            # vs.show_mesh(pred_vertices_graphcnn_431.squeeze().cpu().numpy(), gt3d=gt_keypoints_3d.squeeze().cpu().numpy(),
            #              show_gt3d=True, name='gt_kp+v')
            # Get 14 predicted joints from the non-parametic mesh
            pred_keypoints_3d = torch.matmul(J_regressor_batch, pred_vertices_graphcnn_431)
            pred_pelvis = pred_keypoints_3d[:, [0], :].clone()
            pred_keypoints_3d = pred_keypoints_3d[:, cfg.H36M_TO_J14, :]
            pred_keypoints_3d = pred_keypoints_3d - pred_pelvis

            # vs.show_mesh(pred_vertices_graphcnn_431.squeeze().cpu().numpy(), kp3d=pred_keypoints_3d.squeeze().cpu().numpy(),
            #              gt3d=gt_keypoints_3d.squeeze().cpu().numpy(), show_gt3d=True, show_kp=True, name='pre_kp+v')

            # vs.show_mesh(V=pred_vertices_graphcnn_431.squeeze().cpu().numpy(), gt_V=gt_vertices.squeeze().cpu().numpy(),
            #              kp3d=pred_keypoints_3d.squeeze().cpu().numpy(), gt3d=gt_keypoints_3d.squeeze().cpu().numpy(),
            #              show_gtV=True, show_gt3d=True, show_kp=True, name='gt_v+pre_v+gt_kp+pre_kp')

            # Get 14 predicted joints from the SMPL mesh
            pred_keypoints_3d_smpl = torch.matmul(J_regressor_batch, pred_vertices_hmr)
        ...
        if eval_shape:
            se = torch.sqrt(((pred_vertices_graphcnn_431 - gt_vertices) ** 2).sum(dim=-1)).mean(dim=-1).cpu().numpy()
        ...
        if eval_masks or eval_parts:
            mask, parts = renderer(pred_vertices_graphcnn_431, pred_camera_graphcnn_431)
        ...
        if eval_masks:
            ...
            for i in range(curr_batch_size):
                ...
                # Evaluation consistent with the original UP-c3D code

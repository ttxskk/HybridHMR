...
from models.graph_unet import GraphCNN
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.img_res = 224
        self.graphcnn = GraphCNN(self.mesh._A, num_channels=self.options.num_channels,
                                        num_layers=self.options.num_layers).to(self.device)

        self.keypoint_weight = torch.tensor(
            [5, 2, 0.5, 0.5, 2, 5, 5, 2, 0.5, 0.5, 2, 5, 1, 2, 1, 1, 1, 1, 2, 1, 1, 1, 1, 1], dtype=torch.float).to(
            self.device)

        self.optimizer = torch.optim.Adam(
            params= list(self.cnn_stage.parameters()) + list( self.graphcnn.parameters()) ,
        ...
        self.models_dict = {'cnn_stage': self.cnn_stage, 'graph_cnn': self.graphcnn}
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs,
            ...
            # self.scheduler.step()
            train_gen = tqdm(train_data_loader, total=len(self.train_ds) // self.options.batch_size,
                             initial=train_data_loader.checkpoint_batch_idx)
            for step, batch in enumerate(train_gen, train_data_loader.checkpoint_batch_idx):

                batch = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
                if epoch > -1:
                    out_hmr, out_431 ,out_1723 = self.train_step(batch, epoch)
                    # self.test()
                    train_gen.set_description(
                        'Epoch:%d total_loss:%f total_loss_hmr:%f total_loss_431:%f total_loss_1723:%f kp2d_hmr:%f => kp3d_hmr:%f => loss_shape:%f => pose:%f => betas:%f => loss_shape_431:%f => kp2d_431:%f => kp3d_431:%f => edge_431:%f => normal_431:%f'
                        '=> loss_shape_1723:%f => kp2d_1723:%f => kp3d_1723:%f => edge_1723:%f => normal_1723:%f ' % (
                            epoch, out_hmr[1], out_hmr[2], out_431[1],out_1723[1], out_hmr[6], out_hmr[7], out_hmr[8], out_hmr[9],
                            out_hmr[10], out_431[6], out_431[5], out_431[6], out_431[9], out_431[8],out_1723[6], out_1723[5], out_1723[6], out_1723[9], out_1723[8]))
                else:
                ...
                if self.step_count % self.options.summary_steps == 0:
                    if epoch > -1:
                        ...
                        self.train_summaries_1723(batch, *out_1723)
                    else:
                        self.train_summaries_hmr(batch, *out_hmr)
    ...
    def train_step(self, input_batch, epoch):
        ...
        loss_hmr = loss_keypoints_hmr +  10*loss_keypoints_3d_hmr + loss_shape_hmr + loss_hmr_pose + loss_hmr_betas

        if epoch > -1:
            # resnet_feature = self.resnet(images)
            loss_shape_431, loss_keypoints_431, loss_keypoints_3d_431, loss_edge_431, loss_normal_431, vertices_431_6890, pred_keypoints_2d_431, pred_camera_431\
            ,loss_shape_1723, loss_keypoints_1723, loss_keypoints_3d_1723, loss_edge_1723, loss_normal_1723, vertices_1723_6890, pred_keypoints_2d_1723, pred_camera_1723\
            = self.train_graphcnn(gt_keypoints_3d, has_pose_3d, gt_keypoints_2d, gt_vertices, has_smpl, vertices_hmr.detach(),features_hmr,pose_feature, cam_hmr)
            loss_431 =  loss_shape_431 + loss_keypoints_431 +   10*loss_keypoints_3d_431+ 0.01*loss_normal_431
            loss_1723 = loss_shape_1723 + loss_keypoints_1723 +   10*loss_keypoints_3d_1723 +   0.01*loss_normal_1723
            loss = loss_431 + loss_hmr +loss_1723
            ...
            args_431 = [gt_vertices, loss_431, pred_camera_431, vertices_431_6890, pred_keypoints_2d_431,
                        loss_keypoints_431,
                        10*loss_keypoints_3d_431, loss_shape_431, 0.01*loss_normal_431, loss_edge_431]
            args_1723 = [gt_vertices, loss_1723, pred_camera_1723, vertices_1723_6890, pred_keypoints_2d_1723,
                        loss_keypoints_1723,
                        10*loss_keypoints_3d_1723, loss_shape_1723, 0.01*loss_normal_1723, loss_edge_1723]
            out_args_hmr = [arg.detach() for arg in args_hmr]
            out_args_431 = [arg.detach() for arg in args_431]
            out_args_1723 = [arg.detach() for arg in args_1723]
            return out_args_hmr, out_args_431 ,out_args_1723
        else:
            loss = loss_hmr
            self.optimizer.zero_grad()
            loss.backward()
    ...
    def train_graphcnn(self, gt_keypoints_3d, has_pose_3d, gt_keypoints_2d, gt_vertices, has_smpl, vertices,
                               global_features, pose_feature, cam):
            batch_size = vertices.shape[0]
            att_features = torch.matmul(self.graphcnn.W_431, pose_feature).permute(0, 2, 1)
            ...
            pred_ver_431,pred_ver_1723 = self.graphcnn(image_encode_431)
            # 5'. hmr_camera 和 graphCMR_camera 相加
            ...
            # 7. 利用回归器得到关键点参数
            pred_keypoints_3d_431 = self.smpl.get_joints(pred_ver_431)
            # 8. 关键点正交投影得到2d关键点
            pred_keypoints_2d_431 = orthographic_projection(pred_keypoints_3d_431, cam)[:, :, :2]
            loss_shape_431 = self.shape_loss(pred_ver_431, gt_vertices, has_smpl)
            ...
            loss_edge_431 = self.edge_losses(pred_ver_431, gt_vertices)
            loss_normal_431 = self.normal_loss(pred_ver_431, gt_vertices)

            pred_keypoints_3d_1723 = self.smpl.get_joints(pred_ver_1723)
            # 8. 关键点正交投影得到2d关键点
            pred_keypoints_2d_1723 = orthographic_projection(pred_keypoints_3d_1723, cam)[:, :, :2]
            loss_shape_1723 = self.shape_loss(pred_ver_1723, gt_vertices, has_smpl)
            loss_keypoints_1723 = self.keypoint_loss(pred_keypoints_2d_1723, gt_keypoints_2d)
            loss_keypoints_3d_1723 = self.keypoint_3d_loss(pred_keypoints_3d_1723, gt_keypoints_3d, has_pose_3d)
            loss_edge_1723 = self.edge_losses(pred_ver_1723, gt_vertices)
            loss_normal_1723 = self.normal_loss(pred_ver_1723, gt_vertices)


            return loss_shape_431, loss_keypoints_431, loss_keypoints_3d_431, loss_edge_431, loss_normal_431, pred_ver_431, pred_keypoints_2d_431, cam\
                ,loss_shape_1723, loss_keypoints_1723, loss_keypoints_3d_1723, loss_edge_1723, loss_normal_1723, pred_ver_1723, pred_keypoints_2d_1723, cam
    ...
    def test(self, batch_size=32, log_freq=20):
        ...
        for step, batch in enumerate(test_generator):
            ...
            with torch.no_grad():
                ...
                pred_vert_431, pred_vert_1723 = self.graphcnn(image_enc)
                # 431 上采样到6890
                # pred_vertices_431_6890 = self.mesh.upsample(pred_vertices_graphcnn_431.transpose(1, 2), 2, 0)

                # 2. 训练resnet用于graphcnn_1723特征
                # resnet_feature = self.resnet(images)
                # 3 把431上采样到1723 输出(B, 3, 1723）
                # pred_vertices_graphcnn_1732 = self.mesh.upsample(pred_vertices_graphcnn_431.transpose(1, 2), 2,
                #                                                  1).permute(0, 2, 1)
                # 4. 把resnet特征expand到(B,2048,1723)
                # resnet_feature_enc = resnet_feature.view(batch_size, 2048, 1).expand(-1, -1,
                #                                                                      pred_vertices_graphcnn_1732.shape[
                #                                                                          -1])
                # 5. 把1732顶点3d坐标和feature cat起来输出(B, 2054, 1723)
                # res_enc = torch.cat([pred_vertices_graphcnn_1732, resnet_feature_enc], dim=1)
                # pred_vertices_graphcnn_1723, pred_camera_graphcnn_1723 = self.graph_1723(res_enc)

                # vs.show_mesh(pred_vertices_graphcnn_431.transpose(1, 2).squeeze().cpu().numpy(), name='gt_kp+v')
                # pred_vertices_1723_6890 = self.mesh.upsample(pred_vertices_graphcnn_1723.transpose(1, 2), 1, 0)

            # 3D pose evaluation
            if eval_pose:
                # Regressor broadcasting
                J_regressor_batch = J_regressor[None, :].expand(pred_vert_431.shape[0], -1, -1).to(device)
                ...
                pred_keypoints_3d_1723 = torch.matmul(J_regressor_batch, pred_vert_1723)

                pred_kp2d_1723 = orthographic_projection(pred_keypoints_3d_1723, pred_cam_hmr)
                kp_2d_err_1723 = torch.sqrt(((pred_kp2d_1723[:, cfg.H36M_TO_J14, :] - gt_keypoints_2d) ** 2).sum(dim=-1)).mean(
                    dim=-1).cpu().numpy()
                kp2d_1723.append(torch.sqrt(((pred_kp2d_1723[:, cfg.H36M_TO_J14, :] - gt_keypoints_2d) ** 2).sum(dim=-1)).mean(
                    dim=0).cpu().numpy())

                pred_pelvis_1723 = pred_keypoints_3d_1723[:, [0], :].clone()
                pred_keypoints_3d_1723 = pred_keypoints_3d_1723[:, cfg.H36M_TO_J14, :]
                pred_keypoints_3d_1723 = pred_keypoints_3d_1723 - pred_pelvis_1723


                #431
                pred_keypoints_3d_431 = torch.matmul(J_regressor_batch, pred_vert_431)
                ...
                error_1723 = torch.sqrt(((pred_keypoints_3d_1723 - gt_keypoints_3d) ** 2).sum(dim=-1)).mean(dim=-1).cpu().numpy()
                kp3d_1723.append(torch.sqrt(((pred_keypoints_3d_1723 - gt_keypoints_3d) ** 2).sum(dim=-1)).mean(dim=0).cpu().numpy())
                ...
                self.test_summaries_mesh_431(batch, pred_cam_hmr, pred_vert_431, pred_kp2d_431)
                self.test_summaries_mesh_1723(batch, pred_cam_hmr, pred_vert_1723, pred_kp2d_1723)
                self.test_summaries_mesh_hmr(batch, pred_cam_hmr, pred_vertices_hmr, pred_kp2d_hmr)

            # Shape evaluation (Mean per-vertex error)
            if eval_shape:
                se = torch.sqrt(((pred_vert_431 - gt_vertices) ** 2).sum(dim=-1)).mean(
                    dim=-1).cpu().numpy()
            ...
            test_generator.set_description(
                ' Test  %d/%d ===>MPJPE_431: %f ===>recon_err_431: %f ===>MPJPE_1723: %f===>recon_err_1723:  %f ===> MPJPE_HMR: %f ===>recon_err_HMR: %f'
        ...
        return  1000 * mpjpe_431.mean(), 1000 * recon_err_431.mean(), 1000 * mpjpe_1723.mean(), 1000 * recon_err_1723.mean(),1000 * mpjpe_hmr.mean(), 1000 * recon_err_hmr.mean()

    def test_summaries_mesh_431(self, input_batch,cam_431_, vertices_431_, kp2d):
        ...
        # 记录图像
        self.summary_writer.add_image('test_rend_imgs_431_pred', rend_imgs_431, self.step_count)

    def test_summaries_mesh_1723(self, input_batch,cam_431_, vertices_431_, kp2d):
        gt_keypoints_2d = input_batch['keypoints'].cpu().numpy()
        rend_imgs_431 = []
        batch_size = vertices_431_.shape[0]
        for i in range(min(batch_size, 4)):
        ...
        rend_imgs_431 = make_grid(rend_imgs_431, nrow=8)
        # 记录mesh
        self.summary_writer.add_mesh('test_vertices_1723', vertices=vertices_431_[:3],
                                     faces=(torch.tensor(self.faces.expand(3, -1, -1))).cpu().numpy(),
                                     global_step=self.step_count)
        # 记录图像
        self.summary_writer.add_image('test_rend_imgs_1723_pred', rend_imgs_431, self.step_count)

    def test_summaries_mesh_hmr(self, input_batch,cam_431_, vertices_431_, kp2d):
        gt_keypoints_2d = input_batch['keypoints'].cpu().numpy()
        rend_imgs_431 = []
        batch_size = vertices_431_.shape[0]
        for i in range(min(batch_size, 4)):
        ...
        self.summary_writer.add_mesh('test_vertices_hmr', vertices=vertices_431_[:3],
                                     ...
                                     global_step=self.step_count)
        # 记录图像
        self.summary_writer.add_image('test_rend_imgs_hmr_pred', rend_imgs_431, self.step_count)
    ...
    def train_summaries_hmr(self, input_batch, gt_vertices, total_loss, loss_hmr, cam_hmr_, vertices_hmr_, kp2d,
        ...
        # 记录mesh
        self.summary_writer.add_mesh('vertices_gt', vertices=gt_vertices[:3],
                                     faces=(torch.tensor(self.faces.expand(3, -1, -1))).cpu().numpy(),
                                     global_step=self.step_count)
        ...
        self.summary_writer.add_image('rend_imgs_gt', rend_imgs_gt, self.step_count)
        # 记录loss

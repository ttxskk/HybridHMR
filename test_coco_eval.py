...
# from models.cmr_1723 import CMR
...
parser.add_argument('--regressor', type=str, choices=['hmr', 'hybrid_cmr'], default='hybrid_cmr',
                    help='Name of the SMPL regressor.')
parser.add_argument('--cfg_file', type=str, default='data/models/model_config.json', help='config file path for PyMAF.')
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    # Load SMPL model
    smpl_neutral = SMPL(cfg.SMPL_FILE).to(device)
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
            ...
            elif options.regressor == 'hybrid_cmr':
                pred_vertices_graphcnn_431, pred_vertices_hmr, pred_cam_hmr, pred_camera_graphcnn_431, pred_rotmat, pred_shape = model(
                    images)
            ...
            # pred_J24 = torch.matmul(J_regressor_batch, pred_vertices_graphcnn_431)
            # hmr_verts = smpl_neutral(pred_rotmat, pred_betas)
            pred_J24 = smpl_neutral.get_train_joints(pred_vertices_graphcnn_431)
            # pred_J24 = smpl_neutral.get_train_joints(pred_vertices_graphcnn_431)
            ...
            pred_cam_t = torch.stack([pred_camera_graphcnn_431[:, 1],
                                      pred_camera_graphcnn_431[:, 2],
                                      2 * 5000. / (img_res * pred_camera_graphcnn_431[:, 0] + 1e-9)], dim=-1)
            camera_center = torch.zeros(len(pred_JCOCO), 2, device=pred_camera_graphcnn_431.device)
            pred_keypoints_2d = perspective_projection(pred_JCOCO,
                                                       rotation=torch.eye(3, device=pred_camera_graphcnn_431.device).unsqueeze(
            ...
            if options.vis_demo:
                ...
                vis_smpl_iuv(images_vis.cpu().numpy(), pred_camera.cpu().numpy(), pred_vertices_graphcnn_431.cpu().numpy(),
                             smpl_neutral.faces, None,
...
if __name__ == '__main__':
    ...
    # from utils.cfgs import parse_args
    # parse_args(args)
    from models.cmr_431 import CMR
    from utils.mesh import Mesh
    if args.regressor == 'hybrid_cmr':
        mesh = Mesh()
    ...
    model.eval()
    dataset = COCODataset(options, args.dataset, 'val2014', is_train=False)

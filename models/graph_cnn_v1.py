...
class GraphCNN(nn.Module):
    ...
    def project(self, img_shape, img_features, kp3d, cam):
        keypoint = orthographic_projection(kp3d, cam)
    ...
    def forward(self, image, keypoints):
        ...
        keypoint = ((keypoints[:, :, :2] + 1) * 0.5) * 224  # ????


        # local feature
        img_features = []
        for img_feature in image_resnet:
            img_features.append(self.project(img_shape, img_feature, keypoint,cam))
        ...
        att_features = torch.matmul(self.W_431, cat_feature).permute(0, 2, 1)
        att_features_0_ = torch.cat([vertices_0, att_features], 1)
        ...
        att_features_1 = torch.matmul(self.W_1723, cat_feature).permute(0, 2, 1)

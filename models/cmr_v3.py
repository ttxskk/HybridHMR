...
from models import  SMPLParamRegressor, SMPL

from models.graph_cnn_v3 import GraphCNN

class CMR(nn.Module):

    def __init__(self, mesh, num_layers, num_channels, pretrained_checkpoint=None):
        super(CMR, self).__init__()
        self.graph_cnn = GraphCNN(mesh._A, mesh.ref_vertices.t(),
    ...
    def forward(self, image,keypoints, train_graph_cnn=True, train_smpl_param_regressor=True, detach=True):
        ...
        if not train_graph_cnn:
            with torch.no_grad():
                pred_vertices_sub, camera = self.graph_cnn(image,keypoints)
        else:
            pred_vertices_sub, camera = self.graph_cnn(image,keypoints)
        ...
        x = torch.cat([x, self.mesh.upsample(self.mesh.ref_vertices,2,1)[None, :, :].expand(batch_size, -1, -1)], dim=-1)

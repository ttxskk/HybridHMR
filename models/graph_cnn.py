...
from models.resnet_ import resnet50
...
class GraphCNN(nn.Module):
    ...
    def forward(self, image):
        ...
        vs.show_mesh(ref_vertices.permute(0, 2, 1).squeeze().cpu().numpy(), name='ref_V_1700')
        ...
        x = self.gc(x)
        shape = self.shape(x)
        vs.show_mesh(shape.permute(0, 2, 1).squeeze().cpu().numpy(), name='shape_V_1700')
